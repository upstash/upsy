...
/*
https://upsy-next.vercel.app/api/event
    ...
    try {
        async function fetchMessages(cursor) {
            ...
            if (result.ok && result.messages) {
                ...
                await llm.addDocuments(messagesArray, channelId, "slack-channel-history");
...
(async () => {
    ...
    app.message(/.*/, async ({event , say}) => {
        ...
        if (event.client_msg_id) {
            ...
            if (isDuplicate == 0) {
                console.log("duplicate message received, skipping:" + event.client_msg_id);
                console.log(event)
        ...
        if(event.channel_type === "im" ) {
            response = await llm.query("im", event.text, event.channel, event.user, true);
            ...
            if (isItQuestion || messageLowerCase.includes("upsy")) {
                if (messageLowerCase.includes("upsy")) {
                    ...
                    response = await llm.query("channel", event.text, event.channel, event.user, true);
                }
                console.log("answer:"+response)
                if(response && response.trim() !== "NONE") {
                    await say(response);
        ...
        if ( !isItQuestion ){
            await llm.addDocument( { type: "slack-message", channelId: event.channel, content:msg }, msg);
        }
